...
class ShortPlayMonitorex(_PluginBase):
    ...
    # 插件配置项ID前缀 - 改成唯一的，避免和原版冲突
    plugin_config_prefix = "shortplaymonitorex_"  # 您已经改好了
    # 加载顺序 - 稍微改一下避免冲突（可选）
    plugin_order = 27
    # 可使用的用户级别
    auth_level = 1
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'info',
                                            'variant': 'tonal',
                                            'text': '默认从tmdb刮削，刮削失败则从PT站刮削。当重命名方式为smart时，如站点管理已配置AGSV、ilolicon、ptskit，则优先从站点获取短剧封面。'
                                        }
                                    }
                                ]
                            }
